...
//                비밀번호를 저장
            ...
            if (setPW == checker) {
                Toast.makeText(
                    this@SettingPassword,
                    "[ This password is the same as before. ]",
                    Toast.LENGTH_LONG
                ).show()
...
//                    토스트 내용이 바뀌는 경우 다음 주석이 있는 부분까지 아래와 같이 처리
                ...
                val tmp ="Your password has been changed"
...
//                입력자판 내림
    ...
    override fun onBackPressed() {
        Toast.makeText(this@SettingPassword, "[ Password change has been canceled. ]", Toast.LENGTH_SHORT)
            .show()
        //                입력자판 내림
        val inputMethodManager =
            getSystemService(Activity.INPUT_METHOD_SERVICE) as InputMethodManager
